...
export function CardComponent({ id, icon, heading, content }: { id: number, icon: React.ReactNode, heading: string | undefined, content: string}) {
  return (
    <Card className="w-[180ox] md:w-[170px] lg:w-[200px] bg-gray-50 outline-none border-none group hover:bg-[#5D0265]/75 transition-all">
      <CardHeader>
        <CardTitle className="text-gray-800 text-md font-bold group-hover:text-gray-50 transition-all">
          <div className=" flex justify-center items-center w-8 h-8 rounded-lg bg-gray-100 group-hover:bg-gray-100/30 mb-1 transition-all">
            {icon}
          </div>
          0{id}. {heading}
        </CardTitle>
      ...
      <CardContent>
        <p className="text-gray-500 text-[9px] leading-3 tracking-tight group-hover:text-gray-50 transition-all">{content}</p>
      </CardContent>
